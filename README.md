...
```bash
# 1. put the weave CLI on your PATH (once per machine)
git clone https://github.com/OskarAndreasBerg-Procano/weave.git
cp weave/weave ~/bin/weave && chmod +x ~/bin/weave    # macOS / Linux (ensure ~/bin is on PATH)
#    Windows: copy the `weave` file to e.g. C:\bin\weave and call it as `python C:\bin\weave ...`

# 2. wire it into a project (once per project)
cd your-project && weave init

# 3. restart Claude Code so it loads the weave MCP server + hooks
```

Commit `.weave/graph.json` — it's the shared memory. The per-machine wiring (`.mcp.json`,
`.claude/settings.local.json`) is generated by `weave init` and gitignored.
...
- **No `weave_*` MCP tools after init.** Did you restart the Claude Code chat? `.mcp.json` is read at session start only.
